...
/* ── Groups List View ── */
const GroupsList = ({ groups, onNew, onEdit, onDelete, onViewDetails }) => (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                {groups.map(group => (
                    <div key={group.id} className="group relative bg-white border border-slate-200 rounded-xl p-5 hover:border-indigo-500/50 hover:shadow-lg hover:shadow-indigo-500/10 transition-all duration-300">
                        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => onEdit(group)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"><Edit2 size={14} /></button>
                            <button onClick={() => onDelete(group.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={14} /></button>
                        </div>
                        <div onClick={() => onViewDetails(group.id)} className="cursor-pointer">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-100">{group.total_minutes || 0} m</span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
const GroupsTab = ({ groups, users, onSave, onDelete, showNotif, formatDate }) => {
    ...
    return <GroupsList groups={groups} onNew={handleNew} onEdit={handleEdit} onDelete={handleDelete} onViewDetails={loadDetails} />;
};
